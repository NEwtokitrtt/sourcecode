...
public class ShareFragmentActivity extends Fragment {
	...
	private void initializeLogic() {
		...
		if (stpl.equals("on")) {
			// Call ReferLink directly if needed
			_ReferLink();
		} else {
			if (isAdded() && getActivity() != null) { // Add null checks
				link.setText("https://digitalareagrow.xyz/cashluto/source.php?ref=".concat(tx_ref.getText().toString()).trim());
			}
		}
	}
	...
	public void _ReferLink() {
		if (!isAdded() || getActivity() == null) {
			return; // Don't proceed if Fragment is not properly attached
		}
		...
		// Replace Timer tasks with Handler for safer UI updates
		Handler handler = new Handler(Looper.getMainLooper());
		
		// Check for stored link periodically
		handler.postDelayed(new Runnable() {
			@Override
			public void run() {
				if (!isAdded() || getActivity() == null) {
					return; // Don't proceed if Fragment is detached
				}
				
				if (!ref_link.getString("link", "").isEmpty()) {
					...
					Task<ShortDynamicLink> shortLinkTask = FirebaseDynamicLinks.getInstance().createDynamicLink()
						...
						.addOnCompleteListener(Executors.newSingleThreadExecutor(), task -> {
							if (task.isSuccessful() && isAdded() && getActivity() != null) {
